...
# --- 스트리밍 콜백 핸들러 ---
# Langchain에서 LLM 출력을 토큰 단위로 받아서 처리할 수 있게 해주는 기능
class StreamingCallbackHandler(BaseCallbackHandler):
    """토큰 단위로 실시간 출력하는 콜백 핸들러"""
    
    def __init__(self):
        self.tokens = []
        self.is_agent_output = False
...
agent_executor = AgentExecutor(
    ...
    tools=tools, 
    verbose=True,  # 디버그 로그
    handle_parsing_errors=True, # 파싱 에러 시 자동으로 재시도
    max_iterations=5 # 최대 5번의 도구 호출 허용
)
...
def run_multiturn_conversation():
    ...
    print("=" * 70)
    print("Agentic RAG 시스템 시작 (멀티턴 대화 + 토큰 단위 스트리밍)")
    print("=" * 70)
    print("\n종료: 'quit', 'exit', '종료' 입력")
